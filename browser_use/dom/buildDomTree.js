(
  ...
  function isOTAInteractiveElement(element){
		// Define explicit disable attributes and properties
		const OTAInteractiveElementTags = new Set([
			'ota-use-interactive-target'
			]);
	
		// Check for explicit disable attributes
		for (const otaTag of OTAInteractiveElementTags) {
			if (
				element.hasAttribute(otaTag) ||
				element.getAttribute(otaTag) === '1'  // instead of 'true' or ''
			  ) {
				console.log(66666, element);
				return true;
			  }
		}
		return false;
  ...
  function buildDomTree(node, parentIframe = null) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
	  nodeData.isOTAInteractiveElement = isOTAInteractiveElement(node);
	  if(nodeData.isOTAInteractiveElement){
