...
class BrowserContext:
	...
	@time_execution_async('--input_text_element_handle')
	async def _input_text_element_handle(self, element_handle: ElementHandle, text: str):
		"""
		Input text into an element with proper error handling and state management.
		Handles different types of input fields and ensures proper element state before input.
		"""
		try:
			...
			logger.debug(f'❌  Failed to input text into element: {repr(element_handle)}. Error: {str(e)}')
			raise BrowserError(f'Failed to input text into index {element_handle}')
