...
from browser_use.controller.views_selector import (
	InputTextBySelectorAction
)
...
class Controller(Generic[Context]):
	def __init__(
		...
		@self.registry.action(
			'Input text into a input interactive element by selector',
			param_model=InputTextBySelectorAction,
		)
		async def input_text_by_selector(params: InputTextBySelectorAction, browser: BrowserContext, has_sensitive_data: bool = False):
			try:
				element_handle = await browser.get_locate_element_by_css_selector(params.selector)
				if element_handle:
					try:
						await browser._input_text_element_handle(element_handle, params.text)
					except Exception:
						return ActionResult(error=str(e))
					msg = f'⌨️  Input {params.text} into selector {params.selector}'
					logger.info(msg)
					return ActionResult(extracted_content=msg, include_in_memory=True)
			except Exception as e:
				traceback.print_exc()
				logger.warning(f'Element not an input with selector {params.selector} - most likely the page changed')
				return ActionResult(error=str(e))
		...
		@self.registry.action(
			description="Select <option> inside a <select> addressed by CSS selector",
			param_model=SelectOptionBySelectorAction,
		)
		async def select_option_by_selector(
			params: SelectOptionBySelectorAction,
			browser: BrowserContext,
		) -> ActionResult:
			"""
			...
			"""
			try:
				element_node = await browser.get_locate_element_by_css_selector(
					params.css_selector
				)
				if not element_node:
					return ActionResult(
						error=f"Select not found: {params.css_selector}"
					)

				# Playwright select_option
				chosen = await element_node.select_option(value=params.value, timeout=1_000)
				msg = f"selected option {params.value} via {params.css_selector} ({chosen})"
				return ActionResult(extracted_content=msg, include_in_memory=True)

			except Exception as e:
				return ActionResult(error=f"selection failed: {e}")
