...
class SystemPrompt:
	def __init__(
		...
		self.system_prompt_path = 'system_prompt_wap_replay.md' if replay_mode == "smart_replay" else 'system_prompt.md'
		prompt = ''
	...
	def _load_prompt_template(self) -> None:
		...
		try:
			...
			with importlib.resources.files('browser_use.agent').joinpath(self.system_prompt_path).open('r') as f:
...
class AgentMessagePrompt:
	...
	def get_user_message(self, use_vision: bool = True, is_smart_replay: bool = False) -> HumanMessage:
...
"""
			...
			state_description = f"""
[Task history memory ends]
[sub-goals start here]
Previous sub-goal: {self.subgoals[0]}
Current sub-goal: {self.subgoals[1]}
[sub-goals end]
[Current state starts here]
The following is one-time information - if you need to remember it write it to memory:
Current url: {self.state.url}
